...
type TournamentCopy = {
  ...
  venue: string; venueValue: string; format: string; prizePool: string; registration: string; registrationValue: string; note: string;
...
const copy: Record<Language, TournamentCopy> = {
  ...
  uk: {
    ...
    venue: "Локація", venueValue: "Відень, Австрія", format: "Формат", prizePool: "Призовий фонд", registration: "Реєстрація", registrationValue: "Відкрита", waitlist: "Лист очікування", closed: "Закрита", note: "Форма відкриється з уже обраною дисципліною.", registerTennis: "Зареєструватися на теніс", registerPadel: "Зареєструватися на падел", processLabel: "Реєстрація", processTitle: "Зрозумілий шлях від першого кліку до першої подачі.",
...
function TournamentDetail({ tournament, title, line, description, date, register, copy: c, registrationHref }: { tournament: PublicTournament; title: string; line: string; description: string; date: string; register: string; copy: TournamentCopy; registrationHref: string }) {
  ...
  return <section className="upcoming-page__detail" id={sport}><div className="upcoming-page__detail-intro"><span>{sport === "tennis" ? "02" : "03"} — {sportLabel}</span><h2>{line}</h2><p>{description}</p></div><div className="upcoming-page__detail-body"><div className={`upcoming-page__texture upcoming-page__texture--${sport}`} aria-hidden="true" /><div><p className="upcoming-page__date">{date}</p><h3>{tournament.name || title}</h3><dl><div><dt>{c.venue}</dt><dd>{tournament.location ?? c.venueValue}</dd></div>{tournament.format_description && <div><dt>{c.format}</dt><dd>{tournament.format_description}</dd></div>}{tournament.prize_pool && <div><dt>{c.prizePool}</dt><dd>{tournament.prize_pool}</dd></div>}<div><dt>{c.registration}</dt><dd>{registrationLabel}</dd></div></dl><p className="upcoming-page__note">{c.note}</p>{tournament.registration_status !== "closed" && <a className="upcoming-page__button" href={registrationHref} onClick={(event) => { event.currentTarget.href = withAttribution(registrationHref); trackEvent("select_tournament", { sport, placement: "tournament_detail" }); }}>{register} <span>↗</span></a>}</div></div></section>;
}
