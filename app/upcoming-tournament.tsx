...
type Tournament = { id: string; slug: string; name: string; location: string | null; starts_at: string | null; status: "published" | "live" };
...
export default function UpcomingTournament({ sport }: { sport: Sport }) {
  ...
  const load = useCallback(async () => {
    const result = await supabase.from("tournaments").select("id,slug,name,location,starts_at,status,sport").eq("sport", sport).in("status", ["published", "live"]).order("starts_at", { ascending: true, nullsFirst: false });
